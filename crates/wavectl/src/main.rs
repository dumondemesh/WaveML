...
struct RunExpect {
    /// Ожидаем деление sample rate на этот коэффициент (например, 2 для λ=2)
    #[serde(default)]
    rate_div: Option<f64>,
    /// Ожидаем сокращение длины трека на этот коэффициент (например, 2 для λ=2)
    #[serde(default)]
    len_div: Option<f64>,
}
...
fn cmd_acceptance(plan: PathBuf, outdir: PathBuf, strict: bool) -> Result<()> {
    ...
    // Пред-проверка: все src из плана существуют?
    ...
    for t in tests {
        ...
        match res {
            Ok(ir) => {
                // Компиляция прошла → PASS-ветка
                let mut test_ok = true;
                let mut note = String::new();

                // Если есть run-спецификация — запускаем граф
                if let Some(run_spec) = t.run.as_ref() {
                    ...
                    // Валидация ожиданий run (rate/len)
