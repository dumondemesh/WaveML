...
BACKUP="${TARGET}.force.bak.$(date -u +%Y%m%d%H%M%S)"

mkdir -p scripts/ci

if [[ -f "$TARGET" ]]; then
  cp "$TARGET" "$BACKUP"
  echo "[force] backup saved: $BACKUP"
fi

# Write clean content (heredoc), no SYN, uses nfid()
cat > "$TARGET" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
if command -v shasum >/dev/null 2>&1; then
  echo "[force] sha256: $(shasum -a 256 "$TARGET" | awk '{print $1}')"
fi
